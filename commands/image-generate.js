    ...
    try {
        ...
        if (!imagePrompt) {
            await sock.sendMessage(chatId, {
                text: 'Please provide a prompt for the image generation.\nExample: .image-generate a beautiful sunset over mountains'
            }, {
                quoted: message
            });
            return;
        ...
        // Make API request
        const response = await axios.get(`https://www.samirxpikachu.run.place/marjia`, {
            params: {
                apikey: 'knightbot',
        ...
        await sock.sendMessage(chatId, {
            text: `❌ Failed to generate image. Please try again later. Error: ${error.message}`
        }, {
            quoted: message
        });
    }
